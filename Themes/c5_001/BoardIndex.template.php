	...
	foreach ( $context['categories'] as $category ) {
		...
		foreach ( $category['boards'] as $board ) {
			...
			if ( $board['can_approve_posts'] && ( $board['unapproved_posts'] || $board['unapproved_topics'] ) ) {
				echo '
						<a href="', $scripturl, '?action=moderate;area=postmod;sa=', ( $board['unapproved_topics'] > 0 ? 'topics' : 'posts' ), ';brd=', $board['id'], ';', $context['session_var'], '=', $context['session_id'], '" title="', sprintf( $txt['unapproved_posts'],
				                                                                                                                                                                                                                                   ...
				                                                                                                                                                                                                                                   $board['unapproved_posts'] ), '" class="moderation_link">(!)</a>';
			}
			...
			if ( ! empty( $board['link_moderators'] ) ) {
				echo '
						<p class="moderators">', count( $board['link_moderators'] ) == 1 ? $txt['moderator'] : $txt['moderators'], ': ', implode( ', ',
				                                                                                                                              $board['link_moderators'] ), '</p>';
			}
			...
			echo '
					...
					<div class="stats">
						<p>', comma_format( $board['posts'] ), ' ', $board['is_redirect'] ? $txt['redirects'] : ( $txt['posts'] . ( $board['posts'] > 1 ? 's' : '' ) ), '
						', $board['is_redirect'] ? '' : '<br> ' . comma_format( $board['topics'] ) . ' ' . ( $txt['board_topics'] . ( $board['topics'] > 1 ? 's' : '' ) ), '
						</p>
					</div>
					<div class="lastpost">';
			...
			if ( ! empty( $board['children'] ) ) {
				...
				foreach ( $board['children'] as $child ) {
					if ( ! $child['is_redirect'] ) {
						$child['link'] = '<a href="' . $child['href'] . '" ' . ( $child['new'] ? 'class="board_new_posts" ' :
								'' ) . 'title="' . ( $child['new'] ? $txt['new_posts'] : $txt['old_posts'] ) . ' bbb(' .
						                 $txt['board_topics'] . ': ' . comma_format( $child['topics'] ) . ', ' . $txt['posts'] . ': ' . comma_format( $child['posts'] ) . ')">' . $child['name'] . ( $child['new'] ? '</a> <a href="' . $scripturl
						                                                                                                                                                                                             . '?action=unread;board=' . $child['id'] . '"
title="' . $txt['new_posts'] . ' (' . $txt['board_topics'] . ': ' . comma_format( $child['topics'] ) . ', ' . $txt['posts'] . ': ' . comma_format( $child['posts'] ) . ')"><span class="new_posts">' . $txt['new'] . '</span>' : '' ) . '</a>';
					} else {
					...
					if ( $child['can_approve_posts'] && ( $child['unapproved_posts'] || $child['unapproved_topics'] ) ) {
						$child['link'] .= ' <a href="' . $scripturl . '?action=moderate;area=postmod;sa=' . ( $child['unapproved_topics'] > 0 ? 'topics' : 'posts' ) . ';brd=' . $child['id'] . ';' . $context['session_var'] . '=' . $context['session_id'] . '" title="' . sprintf( $txt['unapproved_posts'],
...
function template_ic_block_calendar() {
	...
	if ( ! empty( $context['calendar_holidays'] ) ) {
		echo '
				<p class="inline holiday"><span>', $txt['calendar_prompt'], '</span> ', implode( ', ',
		                                                                                     $context['calendar_holidays'] ), '</p>';
	}
...
function template_ic_block_online() {
	...
	// $context['num_guests'] = 0;	$context['num_users_online'] = 2;
	// echo $context['num_guests'],' & ',$context['num_users_online'];

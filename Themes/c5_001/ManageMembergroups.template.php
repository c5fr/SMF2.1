...
function template_add_edit_group_boards_list( $collapse = TRUE ) {
	...
	foreach ( $context['categories'] as $category ) {
		if ( empty( $modSettings['deny_boards_access'] ) ) {
			echo '
									<li class="category">
										<a href="javascript:void(0);" onclick="selectBoards([', implode( ', ',
			                                                                               $category['child_ids'] ), '], \'new_group\'); return false;"><strong>', $category['name'], '</strong></a>
									<ul style="width:100%">';
		} else {
			echo '
									<li class="category">
										...
										<span class="select_all_box">
											...
											<select onchange="select_in_category(', $category['id'], ', this, [', implode( ',',
			                                                                                               array_keys( $category['boards'] ) ), ']);">
												<option>---</option>

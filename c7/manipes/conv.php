...
//var_dump(SMF);
//echo '<h1>Chemin: '.$_POST['path_to'].'</h1>';
// Buy some time
@set_time_limit(600);
...
function loadSettings()
{
	...
	*/
	...
	//var_dump($convert_data['database_support'][0]);	exit;
	...
	if ($command_line && !in_array(strtolower($smcFunc['db_title']), $convert_data['database_support']))
		return print_error('The converter detected that you are using ' . $smcFunc['db_title'] . '. This converter only supports ' . explode (', ', $convert_data['database_support'][0]) . '.', true);
	elseif (!in_array(strtolower($smcFunc['db_title']), $convert_data['database_support']))
	{
		template_convert_above();
		echo 'val: '.$smcFunc['db_title'];
		exit;
...
function doStep0($error_message = null)
{
					...
					<table width="100%" cellpadding="0" cellspacing="0" border="0" align="center">
						<tr>
							...
							<td style="padding-bottom: 1ex;">
								...
								<div style="font-style: italic; font-size: smaller;">', $test_to ? '1/ This may be the right path.' :
		'You will need to change the value in this box.', '</div>
							</td>';
	...
	if (!empty($convert_data['settings']))
		echo '
							...
							<td style="padding-bottom: 1ex;">
								...
								<div style="font-style: italic; font-size: smaller;">', $test_from ? '2/ This may be the right path.' :
			'You will need to change the value in this box.', '</div>
							</td>';
	...
	if (!empty($charsets))
	{
		...
		foreach ($charsets as $name => $id)
			echo '
									<option value="', $id, '"', $id == 'utf8' ? ' selected="selected"' : '', '>', $name, '</option>';
		echo '
								</select>
							</td>
						</tr>';
...
function doStep1()
{
	...
	foreach ($lines as $line_number => $line)
	{
		...
		if (isset($_SESSION['do_steps']))
		{
			...
			//var_dump($_SESSION['do_steps']); var_dump($reverse);die;
...
function doStep3()
{
	...
	if (is_writable(dirname(__FILE__)) && is_writable(__FILE__))
		echo '
				...
				<script type="text/javascript"><!-- // --><![CDATA[function doTheDelete(){var theCheck = document.getElementById ? document.getElementById("delete_self") : document.all.delete_self;
...
function pastTime($substep = null, $force = false)
{
	...
	echo '
			...
			<!-- // --><![CDATA[window.onload = doAutoSubmit;
				var countdown = 3;

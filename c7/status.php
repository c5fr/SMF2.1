...
function get_linux_data() {
	...
	if ( ! empty( $context['load_averages'] ) && preg_match( '~^([^ ]+?) ([^ ]+?) ([^ ]+)~', $context['load_averages'],
	                                                         $matches ) != 0
	) {
		$context['load_averages'] = [ $matches[1], $matches[2], $matches[3] ];
	} elseif ( ( $context['load_averages'] = @`uptime 2>/dev/null` ) != null && preg_match( '~load average[s]?: (\d+\.\d+), (\d+\.\d+), (\d+\.\d+)~i',
	                                                                                        $context['load_averages'],
	                                                                                        $matches ) != 0
	) {
		$context['load_averages'] = [ $matches[1], $matches[2], $matches[3] ];
	} else {
	...
	if ( empty( $context['memory_usage'] ) ) {
		...
		if ( ! empty( $meminfo ) && preg_match( '~hw\.physmem: (\d+)~i', $meminfo, $match ) != 0 ) {
			...
			if ( ! empty( $meminfo ) && preg_match( '~hw\.pagesize: (\d+)~i', $meminfo,
			                                        $match1 ) != 0 && preg_match( '~vm\.stats\.vm\.v_free_count: (\d+)~i',
			                                                                      $meminfo, $match2 ) != 0
			) {
			...
			if ( ! empty( $meminfo ) && preg_match( '~PhysMem: (?:.+?) ([\d\.]+\w) used, ([\d\.]+\w) free~', $meminfo,
			                                        $match ) != 0
			) {
	...
	$check_release = [ 'centos', 'fedora', 'gentoo', 'redhat', 'slackware', 'yellowdog' ];
	...
	if ( ! empty( $processes ) ) {
		...
		for ( $i = 1, $n = count( $processes ) - 1; $i < $n; $i ++ ) {
			...
			$context['running_processes'][ $proc[0] ] = [
				'id'    => $proc[0],
				'cpu'   => $proc[1],
				'mem'   => $proc[2],
				'title' => $proc[4],
		...
		$context['top_memory_usage'] = [ '(other)' => [ 'name' => '(other)', 'percent' => 0, 'number' => 0 ] ];
		$context['top_cpu_usage']    = [ '(other)' => [ 'name' => '(other)', 'percent' => 0, 'number' => 0 ] ];
		foreach ( $context['running_processes'] as $proc ) {
			$id = basename( $proc['title'] );

			if ( ! isset( $context['top_memory_usage'][ $id ] ) ) {
				$context['top_memory_usage'][ $id ] = [ 'name' => $id, 'percent' => $proc['mem'], 'number' => 1 ];
			} else {
			...
			if ( ! isset( $context['top_cpu_usage'][ $id ] ) ) {
				$context['top_cpu_usage'][ $id ] = [ 'name' => $id, 'percent' => $proc['cpu'], 'number' => 1 ];
			} else {
...
function get_windows_data() {
	...
	if ( ! empty( $processes ) ) {
		...
		foreach ( $processes as $proc ) {
			...
			$context['running_processes'][ $proc[1] ] = [
				'id'        => $proc[1],
				'cpu_time'  => $proc[7],
				'mem_usage' => $proc[4],
				'title'     => $proc[0],
...
function get_mysql_data() {
	...
	$request                    = mysql_query( "
		SHOW VARIABLES" );
	$context['mysql_variables'] = [ ];
	while ( $row = @mysql_fetch_row( $request ) ) {
		$context['mysql_variables'][ $row[0] ] = [
			'name'  => $row[0],
			'value' => $row[1],
		];
	}
	...
	$context['mysql_status'] = [ ];
	while ( $row = @mysql_fetch_row( $request ) ) {
		$context['mysql_status'][ $row[0] ] = [
			'name'  => $row[0],
			'value' => $row[1],
		];
	}
	...
	$context['mysql_statistics'] = [ ];

	if ( isset( $context['mysql_status']['Connections'], $context['mysql_status']['Uptime'] ) ) {
		$context['mysql_statistics'][] = [
			'description' => 'Connections per second',
			'value'       => $context['mysql_status']['Connections']['value'] / max( 1,
			                                                                         $context['mysql_status']['Uptime']['value'] ),
		];
	}
	...
	if ( isset( $context['mysql_status']['Bytes_received'], $context['mysql_status']['Uptime'] ) ) {
		$context['mysql_statistics'][] = [
			...
			'value'       => ( $context['mysql_status']['Bytes_received']['value'] / max( 1,
			                                                                              $context['mysql_status']['Uptime']['value'] ) ) / 1024,
		];
	}
	...
	if ( isset( $context['mysql_status']['Bytes_sent'], $context['mysql_status']['Uptime'] ) ) {
		$context['mysql_statistics'][] = [
			...
			'value'       => ( $context['mysql_status']['Bytes_sent']['value'] / max( 1,
			                                                                          $context['mysql_status']['Uptime']['value'] ) ) / 1024,
		];
	}
	...
	if ( isset( $context['mysql_status']['Questions'], $context['mysql_status']['Uptime'] ) ) {
		$context['mysql_statistics'][] = [
			...
			'value'       => $context['mysql_status']['Questions']['value'] / max( 1,
			                                                                       $context['mysql_status']['Uptime']['value'] ),
		];
	}
	...
	if ( isset( $context['mysql_status']['Slow_queries'], $context['mysql_status']['Questions'] ) ) {
		$context['mysql_statistics'][] = [
			...
			'value'       => $context['mysql_status']['Slow_queries']['value'] / max( 1,
			                                                                          $context['mysql_status']['Questions']['value'] ),
		];
	}
	...
	if ( isset( $context['mysql_status']['Opened_tables'], $context['mysql_status']['Open_tables'] ) ) {
		$context['mysql_statistics'][] = [
			...
			'value'       => $context['mysql_status']['Opened_tables']['value'] / max( 1,
			                                                                           $context['mysql_status']['Open_tables']['value'] ),
			'setting'     => 'table_cache',
			'max'         => 80,
		];
	}
	...
	if ( isset( $context['mysql_status']['Opened_tables'], $context['mysql_variables']['table_cache']['value'] ) ) {
		$context['mysql_statistics'][] = [
			...
			'value'       => $context['mysql_status']['Open_tables']['value'] / max( 1,
			                                                                         $context['mysql_variables']['table_cache']['value'] ),
			'setting'     => 'table_cache',
			'min'         => 0.5,
			'max'         => 0.9,
		];
	...
	if ( isset( $context['mysql_status']['Key_reads'], $context['mysql_status']['Key_read_requests'] ) ) {
		$context['mysql_statistics'][] = [
			...
			'value'       => $context['mysql_status']['Key_reads']['value'] / max( 1,
			                                                                       $context['mysql_status']['Key_read_requests']['value'] ),
	...
	if ( isset( $context['mysql_status']['Key_writes'], $context['mysql_status']['Key_write_requests'] ) ) {
		$context['mysql_statistics'][] = [
			...
			'value'       => $context['mysql_status']['Key_writes']['value'] / max( 1,
			                                                                        $context['mysql_status']['Key_write_requests']['value'] ),
			'setting'     => 'key_buffer_size',
			'max'         => 0.5,
		];
	}
	...
	if ( isset( $context['mysql_status']['Threads_created'], $context['mysql_status']['Connections'] ) ) {
		$context['mysql_statistics'][] = [
			...
			'value'       => $context['mysql_status']['Connections']['value'] / max( 1,
			                                                                         $context['mysql_status']['Threads_created']['value'] ),
	...
	if ( isset( $context['mysql_status']['Threads_created'], $context['mysql_variables']['thread_cache_size'] ) ) {
		$context['mysql_statistics'][] = [
			...
			'value'       => $context['mysql_status']['Threads_cached']['value'] / max( 1,
			                                                                            $context['mysql_variables']['thread_cache_size']['value'] ),
			'setting'     => 'thread_cache_size',
			'min'         => 0.7,
			'max'         => 0.9,
		];
	...
	if ( isset( $context['mysql_status']['Created_tmp_tables'], $context['mysql_status']['Created_tmp_disk_tables'] ) ) {
		$context['mysql_statistics'][] = [
			...
			'value'       => $context['mysql_status']['Created_tmp_disk_tables']['value'] / max( 1,
			                                                                                     $context['mysql_status']['Created_tmp_tables']['value'] ),
	...
	if ( isset( $context['mysql_status']['Sort_merge_passes'], $context['mysql_status']['Sort_rows'] ) ) {
		$context['mysql_statistics'][] = [
			...
			'value'       => $context['mysql_status']['Sort_merge_passes']['value'] / max( 1,
			                                                                               $context['mysql_status']['Sort_rows']['value'] ),
			'setting'     => 'sort_buffer',
			'max'         => 0.001,
		];
	}
	...
	if ( isset( $context['mysql_status']['Qcache_not_cached'], $context['mysql_status']['Com_select'] ) ) {
		$context['mysql_statistics'][] = [
			...
			'value'       => 1 - $context['mysql_status']['Qcache_hits']['value'] / max( 1,
			                                                                             $context['mysql_status']['Com_select']['value'] + $context['mysql_status']['Qcache_hits']['value'] ),
	...
	if ( isset( $context['mysql_status']['Qcache_lowmem_prunes'], $context['mysql_status']['Com_select'] ) ) {
		$context['mysql_statistics'][] = [
			...
			'value'       => $context['mysql_status']['Qcache_lowmem_prunes']['value'] / max( 1,
			                                                                                  $context['mysql_status']['Com_select']['value'] ),
...
function generate_status() {
	...
	if ( $command_line ) {
		...
		if ( ! empty( $context['top_cpu_usage'] ) ) {
			...
			$temp = [ ];
			foreach ( $context['top_cpu_usage'] as $proc ) {
				$temp[ $proc['percent'] ] = $proc['name'] . ( $proc['number'] > 1 ? ' (' . $proc['number'] . ') ' : ' ' ) . number_format( $proc['percent'],
				                                                                                                                           1 ) . '%';
			}
		...
		if ( ! empty( $context['memory_usage'] ) ) {
			echo 'Memory usage:       ', round( ( $context['memory_usage']['used'] * 100 ) / $context['memory_usage']['total'],
			                                    3 ), '% (', $context['memory_usage']['used'], 'k / ', $context['memory_usage']['total'], 'k)', "\n";
		}
		if ( isset( $context['memory_usage']['swap_used'] ) ) {
			echo 'Swap usage:         ', round( ( $context['memory_usage']['swap_used'] * 100 ) / max( 1,
			                                                                                           $context['memory_usage']['swap_total'] ),
			                                    3 ), '% (', $context['memory_usage']['swap_used'], 'k / ', $context['memory_usage']['swap_total'], 'k)', "\n";
		}
	...
	if ( ! empty( $context['cpu_info'] ) ) {
		echo '
				<tr>
					...
					<td>', strtr( $context['cpu_info']['model'], [ '(R)' => '&reg;' ] ), ' (', $context['cpu_info']['mhz'], 'MHz)</td>
				</tr>';
	}
	...
	if ( ! empty( $context['top_cpu_usage'] ) ) {
		...
		$temp = [ ];
		foreach ( $context['top_cpu_usage'] as $proc ) {
			$temp[ $proc['percent'] ] = htmlspecialchars( $proc['name'] ) . ' <em>(' . $proc['number'] . ')</em> ' . number_format( $proc['percent'],
			                                                                                                                        1 ) . '%';
		}
	...
	if ( ! empty( $context['memory_usage'] ) ) {
		echo '
				<tr>
					...
					<td>
						', round( ( $context['memory_usage']['used'] * 100 ) / $context['memory_usage']['total'],
		                  3 ), '% (', $context['memory_usage']['used'], 'k / ', $context['memory_usage']['total'], 'k)';
		if ( isset( $context['memory_usage']['swap_used'] ) ) {
			echo '<br />
						Swap: ', round( ( $context['memory_usage']['swap_used'] * 100 ) / max( 1,
			                                                                             $context['memory_usage']['swap_total'] ),
			                      3 ), '% (', $context['memory_usage']['swap_used'], 'k / ', $context['memory_usage']['swap_total'], 'k)';
		}
		echo '
		...
		if ( ! empty( $context['mysql_processes'] ) ) {
			...
			foreach ( $context['mysql_processes'] as $proc ) {
				...
				<tr>
					...
					<td><div style="width: 100%; ', strpos( $_SERVER['HTTP_USER_AGENT'],
				                                          'Gecko' ) !== FALSE ? 'max-' : '', 'height: 7em; overflow: auto;"><pre style="margin: 0; border: 1px solid gray;">';
				...
				echo strtr( htmlspecialchars( $clean ), [ "\n" => '<br />', "\r" => '' ] );
...
<html lang="en">
	<head>
		...
		<style type="text/css">
			...
			div#content
			{
				background: url(../../Themes/default/images/backdrop.png) repeat-x #eaeff3;
				padding: 10px 30px;
			}
			div.error_message
			{
			...
			div.panel
			{
				...
				margin: 1ex 0;
				padding: 1.2ex 2ex 1.2ex 1.2ex;
				-webkit-border-radius: 7px;-moz-border-radius: 7px;border-radius: 7px;
				-webkit-box-shadow: inset -2px -1px 9px #ff4a1a;
				-moz-box-shadow: inset -2px -1px 9px #ff4a1a;
				box-shadow: inset -2px -1px 9px #ff4a1a}
			div.panel h2
			{
				margin: 0;
				margin-bottom: 0.5ex;
	...
	<body>
		<div id="header">
			...
			', file_exists( '../Themes/default/images/smflogo.png' ) ? '<a href="http://www.simplemachines.org/" target="_blank"><img
src="../Themes/default/images/smflogo.png" style="width : 250px; float: right;" title="Simple Machines" alt="Simple Machines" border="0" /></a>' : '', '<div title="Wheat Thins">Server Status</div>
